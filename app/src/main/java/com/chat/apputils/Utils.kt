...
//            e.printStackTrace()
    ...
    fun getFormattedDate(timeZone: Date, pattern: String): String? {
        val df = SimpleDateFormat(pattern, Locale("en"))
        return df.format(timeZone)
    }

    fun getFormattedDateLanguageSpecific(timeZone: Date, pattern: String): String? {
...
//        mIntent.putExtra(Intent.EXTRA_SUBJECT, "");
    ...
    fun getUserData(context: Context): UserData? {
        val userData = Utils.getPref(context, Constant.LOGIN_INFO, "")
        if (userData.isNullOrEmpty().not()) {
            return Gson().fromJson<UserData>(
                userData!!, object : TypeToken<UserData>() {}.type
            )
        }
        return null
    }
    ...
    fun isVehicleString(s: String): Boolean {
        if (s.isEmpty().not()) {
            val stringWithOnlyDigits = s.filter { it.isLetterOrDigit() }
            val mPattern: Pattern =
                Pattern.compile("^[A-Z]{2}[0-9]{1,2}[A-Z]*([A-Z]{2})*[0-9]{4}\$")

            val matcher: Matcher = mPattern.matcher(stringWithOnlyDigits.replace(" ", "").trim())
            if (matcher.matches()) {
                return true
            }
        }
    ...
    fun toVehicleString(s: String): String {
        if (s.isEmpty().not() && isVehicleString(s)) {
